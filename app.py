...
def getScore():
    ...
    if(res == 200):
        ...
        ga_1m = [4, 1, 1, 4, 2]
        ga_2m = [4, 3, 1, 2, 3]
        cs_1m = [1, 1, 3, 3, 2, 3, 0, 4, 4, 3, 3, 3, 1, 1, 4, 3]
        cs_1m_nat = [[0.125, 0.125], [7, 7], [5, 5], [1034, 1034], [13.3, 13.5], [19, 19], [13, 13], [7, 7], [6, 6]]
        ...
        name = (soup.findAll("tr")[1].text).strip();
        ...
        for val in cs_1m:
            if(answer[i] == '--'):
                unanswered_1m += 1
            elif(int(answer[i]) == 0 or int(answer[i]) == val):
                pos_score_1m += 1
            else:
                neg_score_1m += 1
            i+=1
